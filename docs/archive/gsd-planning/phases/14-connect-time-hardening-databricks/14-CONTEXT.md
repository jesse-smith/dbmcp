...
- **D-18** *(addendum, 2026-05-11 via research)*: Also remove the third `"main"` fallback at `src/db/connection.py:499` in `_connect_databricks_from_config`. Without this, the named-config path silently defaults to `"main"` before the dialect ever sees the empty catalog, bypassing the IDENT-01 guard. Change `catalog = resolve_env_vars(config.catalog) if config.catalog else "main"` → pass the resolved value (or empty string / None) through; let `_require_databricks_catalog` catch the resulting `ValueError` from `create_engine` and enrich it. Same treatment optional for `schema` default `"default"` — leave schema default alone unless the planner sees a reason; schema is not in IDENT-01 scope.
