...
- [~] T015 [US1] **LIVE (SC-008)**: against the live Databricks warehouse via `dbmcp-test` (reconnect first), call `get_column_info` on a cross-catalog table for one numeric + one string column. Confirm the response now comes via the precomputed DESCRIBE EXTENDED stats (fast path fired) and the keys/shape match what Tier-2 returned before. Capture before/after output in findings.md as the SC-008 evidence. **BLOCKED: probe of `samples.tpch.customer` returned pre-edit behavior (lowercase `data_type` + `total_rows=750000`) — the running `dbmcp-test` MCP server holds pre-edit modules; a `connect_database` reconnect does NOT reimport server Python. Needs a server process RESTART. "before" captured; "after" pending restart. Diagnostic + MSSQL-default-path regression (clean) recorded in findings.md.**
